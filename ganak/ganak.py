...
def main():
    ...
    parser.add_argument("-noPCC", action='store_true', help="turn off probablistic component caching (PCC)",
                        dest='noPCC')
    parser.add_argument("-noCSVSADS", action='store_true', help="turn off CSVSADS branching heuristic",
                        dest='noCSVSADS')
    ...
    parser.add_argument("-noLSO", help="turn off learn and start over heuristic (LSO)", dest='noLSO',
                        action='store_true')
    parser.add_argument("-EDR", help="use exponentially decaying randomness (EDR)", dest='EDR', action='store_true')
    parser.add_argument("-lso ", type=int, default=5000, help="use learn and start over heuristic after LSO decisions",
                        dest='LSO')
    ...
    parser.add_argument("-p", help="calculate projected model counting [Warning! IS will be turned off]",
                        dest='performprojection', action='store_true')
    ...
    f_out = open('outputfile').read()
    n_sol = int(re.findall(r"s mc (\d+)", f_out)[0])
    print(f"{os.path.basename(args.DIMACSCNF)},{n_sol}")
